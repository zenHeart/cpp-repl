...
function getInstance(module, imports) {
  if (typeof module === 'string') {
    return (async () => {
      const wasmUrl = getWasmUrl(module);
...
class App {
  constructor(module, memfs, name, ...args) {
    ...
    this.ready = getInstance(module, { wasi_unstable, env }).then(
      (instance) => {
        this.instance = instance;
...
export class API {
  ...
  async untar(memfs, filename) {
    ...
    const promise = (async () => {
      console.log(`开始解包 ${filename}`);
      const tar = new Tar(await this.readBuffer(filename));
      tar.untar(this.memfs);
      // 添加调试输出
      // @ts-ignore
      console.log('文件系统内容:', this.memfs);
      // this.memfs.exports.Debug_ListFiles();  
    })();
